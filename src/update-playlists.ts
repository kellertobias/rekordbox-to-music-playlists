import { PL_DESCR_SLUG, PLAYLIST_PATH_SEP } from './constants';
...
export const updatePlaylists = (
  ...
  const deletePlaylists: AugmentedMusicPlaylist[] = musicPlaylistKeys
    .filter((mp) => rbPlaylistKeys.indexOf(mp) === -1)
    .map((key) => musicPlaylits[key]);
  const createPlaylists: RekordboxPlaylist[] = rbPlaylistKeys
    .filter((mp) => musicPlaylistKeys.indexOf(mp) === -1)
  ...
  playlistKeys.forEach((key) => {
    ...
    if (!rb) {
      console.log(
        `Playlist ${key} was in Music but not in rekordbox. Cannot update.`
      );
      return;
    }
    // console.log(JSON.stringify({key, mp, rb}, null, 2))
    const tracks = rb.tracks
      .map((t: CommonTrack | null) => {
        return {
          ref: t?.musicRef,
    ...
    const { added, removed } = updateSinglePlaylist(
      mp,
      tracks,
      `${count}/${playlistKeys.length}`
    );
    addedTotal += added;
