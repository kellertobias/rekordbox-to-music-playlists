...
export const loadTracks = (
  rekordboxTracks: Record<CommonTrackId, CommonTrack>
) => {
  startTimer();
  ...
  console.log(
    `[MUSIC]: ${missingTrackIds.length} tracks missing or not matchable in music`
  );
...
export const loadPlaylists = () => {
  ...
  music.playlists().forEach((p) => {
    ...
    playlists[playlistPath.join(PLAYLIST_PATH_SEP)] = {
      playlist: p,
      path: playlistPath,
    };
  });
  ...
  console.log(
    `[MUSIC]: ${matchCount}/${count} playlists are managed by Rekordbox`
  );
  console.log(`[MUSIC]: Loading Playlists took ${getTimer()} seconds`);
